...
import type { SpeciesListItem, SpeciesListResponse } from "../types/SWAPI-types/species.types";
...
const SpeciesPage = () => {
	...
	const {
		...
		isLoading,
		getData
	} = useGet<SpeciesListItem, SpeciesListResponse>();

	const prevQuery = useRef<string | null>(null);
	const [isNewQuery, setIsNewQuery] = useState(false);

	const resourceCategory = "Species"

	useEffect(() => {
		if (prevQuery.current !== query) {
			setIsNewQuery(true);
		}
		getData(SpeciesApi.getSpecies, page, query);
